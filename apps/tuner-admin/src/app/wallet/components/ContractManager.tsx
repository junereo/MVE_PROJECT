...
export default function ContractManager() {
  ...
  return (
    <div className="mt-8 p-6 bg-gray-50 rounded-xl shadow space-y-4">
      ...
      <Input placeholder="Token Address" value={caToken} onChange={e => setCaToken(e.target.value)} />
      <label className="font-semibold">뱃지 컨트랙트 :</label>
      <Input placeholder="Badge Address" value={caBadge} onChange={e => setCaBadge(e.target.value)} />
      <label className="font-semibold">서베이 컨트랙트 :</label>
      <Input placeholder="Survey Address" value={caSurvey} onChange={e => setCaSurvey(e.target.value)} />
      <label className="font-semibold">메타 트랜잭션 컨트랙트 :</label>
      <Input placeholder="Transac Address" value={caTransac} onChange={e => setCaTransac(e.target.value)} />
