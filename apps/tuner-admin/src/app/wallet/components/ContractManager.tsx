...
export default function ContractManager() {
  ...
  const [allowanceTimeoutId, setAllowanceTimeoutId] = useState<NodeJS.Timeout | null>(null);
  ...
  const handleApproveOrRevoke = async () => {
    ...
    if (!isValidEthAddress(approveSpender)) {
      setLoading(false);
      toast.error("오너 주소는 0x로 시작하는 42자 이더리움 주소여야 합니다.");
      return;
    }
    if (!isValidEthAddress(approveTokenAddress)) {
      setLoading(false);
      toast.error("토큰 컨트랙트 주소는 0x로 시작하는 42자 이더리움 주소여야 합니다.");
      return;
    }
    ...
    const summary =
      ...
      `수량: ${approveAmount} (${amountNum === 0 ? '위임 취소' : '위임 승인'})`;
    ...
    try {
      if (amountNum === 0) {
        const res = await axios.post(baseUrl + "/contract/wallet/revoke", {
          spender: approveSpender,
          tokenAddress: approveTokenAddress,
        });
        toast.success("Revoke 성공: " + res.data.txHash);
        setResultMessage(`✅ 위임 취소 성공!\nTxHash: ${res.data.txHash}`);
      } else if (amountNum > 0) {
        const res = await axios.post(baseUrl + "/contract/wallet/approve", {
          spender: approveSpender,
          tokenAddress: approveTokenAddress,
          amount: amountNum.toString(),
        });
        toast.success("Approve 성공: " + res.data.txHash);
        setResultMessage(`✅ 위임 승인 성공!\nTxHash: ${res.data.txHash}`);
      }
    } catch (e: any) {
  ...
  const handleAllowanceCheck = async () => {
    ...
    if (!isValidEthAddress(allowanceOwner) || !isValidEthAddress(allowanceSpender) || !isValidEthAddress(allowanceTokenAddress)) {
      setAllowanceLoading(false);
      toast.error("모든 주소는 0x로 시작하는 42자 이더리움 주소여야 합니다.");
      return;
    }
    try {
      const res = await axios.post(baseUrl + "/contract/wallet/allowance", {
      ...
      setAllowanceResult("조회 실패: " + (e?.response?.data?.error || ""));
    } finally {
    ...
    <div className="mt-8 p-6 bg-gray-50 rounded-xl shadow space-y-4">
      ...
      <div className="mt-8 p-4 bg-white rounded-xl shadow space-y-2 border border-blue-200">
        ...
        <div className="flex gap-2 mb-4">
          <button
            className={`px-4 py-2 rounded-t ${tab === 'approve' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
            onClick={() => setTab('approve')}
          >
            토큰 위임 Approve / Revoke
          </button>
          <button
            className={`px-4 py-2 rounded-t ${tab === 'allowance' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
        ...
        {tab === 'approve' && (
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              className={`border rounded p-2 font-mono ${approveSpender && !isValidEthAddress(approveSpender) ? 'border-red-500' : ''}`}
              value={approveSpender}
              onChange={e => setApproveSpender(e.target.value)}
              maxLength={42}
            />
            {approveSpender && !isValidEthAddress(approveSpender) && (
              <p className="text-red-500 text-xs ml-1">0x로 시작하는 42자 이더리움 주소여야 합니다.</p>
            )}
            <input
              type="text"
              placeholder="토큰 컨트랙트 주소 (0x... 42자)"
              className={`border rounded p-2 font-mono ${approveTokenAddress && !isValidEthAddress(approveTokenAddress) ? 'border-red-500' : ''}`}
              value={approveTokenAddress}
              onChange={e => setApproveTokenAddress(e.target.value)}
              maxLength={42}
            />
            {approveTokenAddress && !isValidEthAddress(approveTokenAddress) && (
              <p className="text-red-500 text-xs ml-1">0x로 시작하는 42자 이더리움 주소여야 합니다.</p>
            )}
            <input
              type="number"
        ...
        {tab === 'allowance' && (
          <div className="flex flex-col gap-2">
            <input
              ...
              className={`border rounded p-2 font-mono ${allowanceOwner && !isValidEthAddress(allowanceOwner) ? 'border-red-500' : ''}`}
            ...
            <input
              ...
              className={`border rounded p-2 font-mono ${allowanceSpender && !isValidEthAddress(allowanceSpender) ? 'border-red-500' : ''}`}
            ...
            <input
              type="text"
              placeholder="토큰 컨트랙트 주소 (0x... 42자)"
              className={`border rounded p-2 font-mono ${allowanceTokenAddress && !isValidEthAddress(allowanceTokenAddress) ? 'border-red-500' : ''}`}
              value={allowanceTokenAddress}
              onChange={e => setAllowanceTokenAddress(e.target.value)}
              maxLength={42}
            />
            <Button className="bg-blue-600 text-white mt-2" onClick={handleAllowanceCheck} disabled={allowanceLoading}>
      ...
      <button className="w-full bg-blue-600 text-white py-2 rounded" onClick={handleContractSave} disabled={saving}>
        {saving ? "변경 중..." : "변경"}
      </button>
    </div>
  );
